<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:75:"/www/web/erp/public_html/public/../application/home/view/sumao/shouhuo.html";i:1514427649;s:68:"/www/web/erp/public_html/public/../application/home/view/layout.html";i:1514539775;}*/ ?>
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
...
function dosearch(){
    ...
    layui.use('table', function(){
        ...
        table.reload('orderTable', {
          url: '<?php echo url('Sumao/getShOrder'); ?>'
...
<table class="layui-table" lay-data="{
  ...
  <thead>
    <tr>
        ...
        <th lay-data="{field:'zhuangchetime', width:'15%'}">装车时间</th>
...
<script type="text/html" id="ss">
  ...
  {{#  } else if(d.status==3) { }}
        <span style="color:green">历史装车单</span>
  {{#  } }}
</script>
...
function xiangqing(data){
  ...
  layui.use('table', function(){
    var table = layui.table;
$.ajax({
  url: "<?php echo url('Sumao/getShXiangqing'); ?>",
  type: 'POST',
  data: {'zcid':data.zcid},
  success:function(res){
    dXq = res.data;
    $("#xiangqingSn").html("装车单号："+'zc-'+data.zcid);
    $("#xiangqingTime").html("装车时间："+data.addtime);
    $("#xiangqingTitle").html("苏州"+data.category+"收货单");
...
<div class="box">
  ...
  <div style="margin-top:15px;" class="xiangqingBtn">
    ...
    <button class="layui-btn layui-btn-warm xiangqingSub" cishu="0">确认</button>
  </div>
</div>
</div>
<script type="text/javascript">
...
function xiangqingSub(zcid){
  ...
  layer.confirm('确认装车单号为：zc-'+zcid+'已全部清点无误并已入库?', {icon: 3, title:'警告'}, function(index){
      $.ajax({
        url: "<?php echo url('Sumao/completeShouhuo'); ?>",
        type: 'POST',
        data: {'zcid':zcid},
        success:function(res){
